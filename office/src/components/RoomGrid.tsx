...
export const RoomGrid = memo(function RoomGrid({ sessions, agents, saiyanTargets, onSelectAgent }: RoomGridProps) {
  ...
  const busyCount = agents.filter(a => a.status === "working" || a.status === "permission").length;

  return (
    <div className="max-w-[1200px] mx-auto px-3 sm:px-6 pt-4 sm:pt-8 pb-12">
      {/* Power bar */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {sessions.map((s) => {
          ...
          const hasBusy = roomAgents.some(a => a.status === "working" || a.status === "permission");
          ...
          return (
            ...
            >
              ...
              {/* Agent grid */}
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-4 p-3 sm:p-5 min-h-[100px] sm:min-h-[140px]">
